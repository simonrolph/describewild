...
```{r}
media_data <- arrange(media_data,series_id,episode_num,date)
...
for (i in creators_data$id){
  page <- readLines("~/describewild/creator_template.html")
  page <- paste(page, collapse = '')
  ...
  for(j in (1:nrow(media_data))[media_data$creator_id == i]){
    ...
    if (!is.na(media_data$series[j])){
      if (new_series != media_data$series[j]){
        medias <- paste0(medias,"<a id=",media_data$series_id[j],"><h3>",media_data$series[j],"</h3></a>")
  ...
  page <- gsub("insert_medias",medias,page)
  page <- gsub("insert_update_date",Sys.Date(),page)
  write(page,file = paste0("~/describewild/creators/",this_creator$id,".html"))
